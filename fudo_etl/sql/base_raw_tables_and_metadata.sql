...
-- #            Para cualquier cliente de Fudo                          #
-- ######################################################################

-- Este script define la estructura de las tablas que almacenan los datos
-- crudos (RAW) de la API de Fudo y los metadatos del proceso ETL.
-- Es común para todos los clientes que utilicen el ETL de Fudo.
...
CREATE TABLE IF NOT EXISTS public.fudo_raw_providers ( -- <--- ¡NUEVA TABLA!
    id_fudo TEXT NOT NULL, 
    id_sucursal_fuente VARCHAR(255) NOT NULL, 
    fecha_extraccion_utc TIMESTAMP WITH TIME ZONE NOT NULL, 
    payload_json JSONB NOT NULL, 
...
CREATE INDEX IF NOT EXISTS idx_fudo_raw_providers_id_sucursal_fecha ON public.fudo_raw_providers (id_fudo, id_sucursal_fuente, fecha_extraccion_utc DESC);
...
CREATE TABLE IF NOT EXISTS public.Rubros (
  id_rubro_fudo TEXT PRIMARY KEY, -- <--- ¡CORRECCIÓN AQUÍ!
...
CREATE TABLE IF NOT EXISTS public.Medio_pago (
  id_payment_fudo TEXT PRIMARY KEY, -- <--- ¡CORRECCIÓN AQUÍ!
...
CREATE TABLE IF NOT EXISTS public.Productos (
  id_product_fudo TEXT, -- <--- ¡CORRECCIÓN AQUÍ!
  id_sucursal VARCHAR(255),
  product_key TEXT PRIMARY KEY, -- Nueva PK
  product_name VARCHAR(255) NOT NULL,
  id_rubro_fudo TEXT, -- <--- ¡CORRECCIÓN AQUÍ!
...
CREATE TABLE IF NOT EXISTS public.Sales_order (
  id_order TEXT PRIMARY KEY, -- <--- ¡CORRECCIÓN AQUÍ!
  amount_tax FLOAT,
...
CREATE TABLE IF NOT EXISTS public.Pagos (
  id TEXT PRIMARY KEY, -- <--- ¡CORRECCIÓN AQUÍ!
  pos_order_id TEXT NOT NULL, -- <--- ¡CORRECCIÓN AQUÍ!
  id_payment TEXT NOT NULL, -- <--- ¡CORRECCIÓN AQUÍ!
...
CREATE TABLE IF NOT EXISTS public.Sales_order_line (
  id_order_line TEXT PRIMARY KEY, -- <--- ¡CORRECCIÓN AQUÍ!
  id_order_fudo TEXT NOT NULL, -- <--- ¡CORRECCIÓN AQUÍ!
  date_order_time TIMESTAMP WITH TIME ZONE NOT NULL,
  date_order DATE NOT NULL,
  id_product_fudo TEXT NOT NULL, -- <--- ¡CORRECCIÓN AQUÍ!
...
SELECT DISTINCT ON (ec.id_fudo, ec.id_sucursal_fuente)
    ...
    (ec.payload_json -> 'attributes' ->> 'name')::VARCHAR(255) AS expense_category_name,
...
-- mv_expenses
           DROP MATERIALIZED VIEW IF EXISTS public.mv_expenses CASCADE;
            ...
            SELECT DISTINCT ON (e.id_fudo, e.id_sucursal_fuente)
                ...
                (e.payload_json -> 'relationships' -> 'user' -> 'data' ->> 'id')::TEXT AS user_id,
                (e.payload_json -> 'relationships' -> 'provider' -> 'data' ->> 'id')::TEXT AS provider_id,
                (e.payload_json -> 'relationships' -> 'receiptType' -> 'data' ->> 'id')::TEXT AS receipt_type_id,
                (e.payload_json -> 'relationships' -> 'cashRegister' -> 'data' ->> 'id')::TEXT AS cash_register_id,
                (e.payload_json -> 'relationships' -> 'paymentMethod' -> 'data' ->> 'id')::TEXT AS payment_method_id,
                (e.payload_json -> 'relationships' -> 'expenseCategory' -> 'data' ->> 'id')::TEXT AS expense_category_id,
...
SELECT DISTINCT ON (pc.id_fudo, pc.id_sucursal_fuente)
    ...
    (pc.payload_json -> 'attributes' ->> 'position')::FLOAT::INTEGER AS "position", -- <--- CORRECCIÓN AQUÍ
    (pc.payload_json -> 'attributes' ->> 'preparationTime')::FLOAT::INTEGER AS preparation_time, -- <--- CORRECCIÓN AQUÍ
...
SELECT
    ...
    (c.payload_json -> 'attributes' ->> 'historicalSalesCount')::FLOAT::INTEGER AS historical_sales_count, -- <--- CORRECCIÓN AQUÍ
    ...
    (c.payload_json -> 'attributes' ->> 'salesCount')::FLOAT::INTEGER AS sales_count, -- <--- CORRECCIÓN AQUÍ
...
SELECT
    ...
    (p.payload_json -> 'attributes' ->> 'position')::FLOAT::INTEGER AS "position", -- <--- CORRECCIÓN AQUÍ
    (p.payload_json -> 'attributes' ->> 'preparationTime')::FLOAT::INTEGER AS preparation_time, -- <--- CORRECCIÓN AQUÍ
...
SELECT
    ...
    (pm.payload_json -> 'attributes' ->> 'maxQuantity')::FLOAT::INTEGER AS max_quantity, -- <--- CORRECCIÓN AQUÍ
...
SELECT
    ...
    (t.payload_json -> 'attributes' ->> 'column')::FLOAT::INTEGER AS "column", -- <--- CORRECCIÓN AQUÍ
    (t.payload_json -> 'attributes' ->> 'number')::FLOAT::INTEGER AS table_number, -- <--- CORRECCIÓN AQUÍ
    (t.payload_json -> 'attributes' ->> 'row')::FLOAT::INTEGER AS "row", -- <--- CORRECCIÓN AQUÍ
...
SELECT
    ...
    (s.payload_json -> 'attributes' ->> 'createdAt')::TIMESTAMP WITH TIME ZONE AS created_at,
    (s.payload_json -> 'attributes' ->> 'people')::FLOAT::INTEGER AS people, -- <--- CORRECCIÓN AQUÍ
